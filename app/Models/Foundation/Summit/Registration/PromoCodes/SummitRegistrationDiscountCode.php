...
class SummitRegistrationDiscountCode extends SummitRegistrationPromoCode
{
    ...
    public function getRuleByTicketType(SummitTicketType $ticket_type){
        try {
            $query = $this->createQuery("SELECT f from models\summit\SummitRegistrationDiscountCodeTicketTypeRule r 
    ...
    public function applyTo(SummitAttendeeTicket $ticket){
        ...
        if($this->amount > 0.0){
            ...
            $amount2Discount = ($this->rate * $ticket->getRawCost()) / 100.00;
        }
        else{
            ...
            if(!is_null($rule) && $rule->getAmount() > 0.0){
                ...
                $amount2Discount =  ($this->amount * $rule->getRate()) / 100.00;
            }
        }

        return $ticket->setDiscount($amount2Discount);
    }
