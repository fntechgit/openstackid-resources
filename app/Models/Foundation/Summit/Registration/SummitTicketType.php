...
class SummitTicketType extends SilverstripeBaseModel
{
    ...
    public function preRemoveHandler(LifecycleEventArgs $args){
       if($this->quantity_sold > 0)
            throw new ValidationException(sprintf("can not delete ticket type %s because has tickets sell", $this->getId()));
    }
    ...
    /**
     * @param \DateTime $sales_start_date
     */
     ...
     * @throws ValidationException
     */
    public function sell(int $qty):int{
    ...
    public function restore(int $qty):int{
        if(($this->quantity_sold - $qty) < 0)
            throw new ValidationException("can not restore a greater qty than sold ");

        $this->quantity_sold  = $this->quantity_sold - $qty;

        Log::info("SummitTicketType::restore quantity_sold ". $this->quantity_sold);
