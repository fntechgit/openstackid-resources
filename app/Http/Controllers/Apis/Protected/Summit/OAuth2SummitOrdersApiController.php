    ...
    extends OAuth2ProtectedController
{
    ...
    public function reserve($summit_id){
        try {
            ...
            return $this->created(SerializerRegistry::getInstance()->getSerializer($order, ISummitOrderSerializerTypes::ReservationType)->serialize('tickets,extra_questions,applied_taxes,badge,tax,ticket_type,type'));
     ...
     * @return mixed
     */
    public function confirm(LaravelRequest $request){

        if(!Request::isJson())
            return $this->error400();

        try {
            $response = $this->payment_api->processCallback($request);
